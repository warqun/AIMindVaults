...
`_INBOX.md` 는 루트에 있다. 디스코드 `#task-queue` 에 던진 메시지가 `poll_inbox.py` 로
`## 대기 중` 에 적재되고, ▶️ 를 누르면 무인 회차가 **한 건**을 집어 처리한다. 지시 맨 앞
`[codex]` 태그가 어느 에이전트로 돌지 정한다. 설치·사용은 `docs/discord-inbox.md`,
스크립트는 `_tools/agent-daemon/`.

| 파일 | 쓰는 주체 | 내용 |
|------|----------|------|
| `_INBOX.md` | **사용자만** | 앞으로 할 일. 형식 없는 한 줄 지시 |
| `_STATUS.md` | **에이전트만** | 현재 위치와 다음 할 일. 구조화된 큐 |

- **에이전트는 `_INBOX.md` 의 "대기 중" 에 항목을 추가하지 않는다.** 물을 것이 있으면 대상 볼트 `_STATUS.md` Blocked 에 남긴다.
- **작업 선택 순서**: `_INBOX.md` 대기 중에 항목이 있으면 그것부터. 없으면 대상 볼트 `_STATUS.md` Next.
- **집을 때 풀어 쓴다.** 한 줄 지시를 실행 가능한 단위로 분해해 대상 볼트 `_STATUS.md` 에 옮기고, 원 항목은 "처리됨" 으로 이동하며 어디로 갔는지 남긴다. 처리됨 항목은 지우지 않는다.
- 항목이 어느 볼트 일인지는 § 볼트 진입 프로토콜로 판정한다. 큐가 루트에 있다고 루트에서 작업하지 않는다.
- `!` 로 시작하는 줄은 제어 명령이라 큐에 안 들어간다. 보이면 처리하지 말고 지운다.
- `_AGENT_TASKS/` 는 긴 지시를 파일로 던지는 두 번째 큐 (규약은 그 폴더 `README.md`). **이 폴더의 작업이 이 폴더에 새 작업 파일을 만들 수 없다.**
