...
### `tags` 규칙 (5-12 PascalCase 통일)

**단일 규칙: 전 태그 PascalCase + 약어 대문자 유지 + 다국어 자유 (어느 언어든 상관없음)**

| 케이스 | 정규 표기 | 예시 |
|--------|----------|------|
| 단일 단어 (영어) | PascalCase 첫 글자 대문자 | `Unity`, `Blender`, `Cooking`, `Research` |
...
**기본 원칙**:
- **Flat 전용**: `/` 슬래시 계층 금지 (`unity/dots` → `UnityDOTS` 자동 변환)
- **단수형**: `systems` X → `System`
- **메타 태그 금지** (`domain`, `Meta`, `(볼트 태그)` 등 자동 박힘 태그) — 박지 않음. owned 추론은 폴더 경로 자동
- **volume_id 정확 일치 태그만 owned** — Unity 볼트 노트가 `Unity` 태그 박으면 자동 owned. `DOTS`/`gameplay` 등은 일반 태그
- **alias 사전**: `Vaults/BasicVaults/CoreHub/.sync/_tools/cli-node/data/tag-aliases.yaml` (Core 정본). 새 alias 필요 시 CoreHub 에서 편집 + `bump-version --broadcast`

**자동 변환 룰 (master-index-build / tag-migrate.js 내장)**:
1. `tag-aliases.yaml` 의 `aliases` 매핑 우선
2. `remove` 리스트 (메타 태그) 일괄 삭제
3. 한국어·일본어·중국어 등 비라틴 문자 포함 → 변환 0 (그대로)
4. PascalCase / 대문자 약어 / vault_id 일치 / 숫자 포함 → 그대로
5. kebab-case / snake_case → PascalCase (단어 경계 마다 대문자)
6. nested `/` → PascalCase 결합 (`vault/idea` → `VaultIdea`)
7. lowercase 단일 단어 → 첫 글자 대문자
8. dedup + UTF-8 보존
...
- **운영·상태 태그만** — `Zk`, `Literature`, `Synthesis`, `MoC`, `Pattern` (정식 ZK 5 타입, **PascalCase 통일**) + `AiDraft` (AI 미승격 초안 — `Contents/_AI_Drafts/` 권장), `Temp` (사람 임시)
- **개념 분류 태그 금지** — 개념은 `[[wikilink]]` + MOC 노트로 표현
- 도메인 식별은 인덱서 `vault` 필드 자동 처리 (도메인명 태그 X)
- 정식 타입과 운영 단계 병기 가능 (`tags: [Zk, AiDraft]`)
- 정본: 해당 볼트 `CLAUDE.md § 태그 규칙` 또는 `Vaults/BasicVaults/BasicDomainVault/CLAUDE.md`

### 볼트별 추가 규칙

- 볼트 `CLAUDE.md` 에 도메인 특수 태그 분류·키워드 추가 선언 가능 (코어 + 볼트별 2계층)
- **단 메타 태그 ("모든 노트에 X 태그 필수" 류) 추가 금지** (5-12 결정)
