...
```
   ...
   - **볼트 생성을 여기서 막지 않는다 (강제).** Obsidian 이 켜져 있으면 등록만 미루고 나머지 단계를 끝낸다. 사용자에게 "지금 다 닫으라" 고 요구하지 않는다 — 작업 흐름을 끊는 비용이 등록 지연 비용보다 크다.
   - **자동 등록 (deferred)**: 루트 `.claude/settings.json` 의 SessionStart 훅이 매 세션 시작 시 `register-vaults --apply` 를 돌린다. Obsidian 이 닫혀 있고 미등록 볼트가 있을 때만 등록하고, 그 외에는 조용히 통과한다. 별도 대기 큐 파일은 두지 않는다 — CLI 가 스캔으로 델타를 계산하므로 **볼트 존재 자체가 상태**다.
   - 즉 실제 흐름: 볼트 생성 (막지 않음) → 사용자가 편할 때 Obsidian 종료 → 다음 세션 시작 시 자동 등록 → Obsidian 실행 시 목록에 이미 존재.
   - 즉시 등록이 필요하면 (지금 바로 그 볼트를 열어야 하는 경우) Obsidian 종료 후 위 명령을 직접 실행한다. `/obsidian-windows` 스킬로 graceful close 가능.
   - 첫 오픈 시 "Trust author and enable plugins" 안내는 그대로 필요하다.
   - apply 결과 자동 백업 (`obsidian.json.bak_YYYYMMDD_HHMMSS`) + `Added : N vault entries` 출력.
   - GUI fallback (Obsidian 볼트 매니저 → "보관함 폴더 열기") 도 가능하지만 CLI 우선.
   - 상세 단계는 `/create-vault` 스킬 § 5 참조.
