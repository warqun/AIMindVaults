...
- workspace 편집은 **AIHubVault에서만**. 다른 볼트는 `node cli.js sync`로 자동 전파.
- `.obsidian/` 변경도 workspace 편집. 개별 볼트에 직접 플러그인 설치하지 않는다.

**workspace 편집 순서 (강제):**
1. 파일 수정
2. **즉시** AIHubVault의 `_WORKSPACE_VERSION.md`에 버전 기록 (당일 최대 +1, `YYYYMMDDNNNN`)
3. 그 다음 테스트·배포·동기화
