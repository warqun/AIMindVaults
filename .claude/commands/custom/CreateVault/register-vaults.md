...
- **Obsidian 완전 종료 상태** — 실행 중이면 차단. 이유 둘: ① live write-back 으로 외부 추가분이 사일런트 revert ② **실행 중인 인스턴스는 새 항목을 어차피 못 읽는다** (Obsidian 은 `obsidian.json` 을 시작 시점에 읽는다 — 2026-08-26 조사 확정). 즉 `--force` 로 강행해도 얻는 게 없다.
- **수동 실행이 필수는 아니다** — 루트 `.claude/settings.json` 의 SessionStart 훅이 매 세션 시작 시 `--apply` 를 자동 실행한다. Obsidian 이 닫혀 있고 미등록 볼트가 있을 때만 등록하고 그 외에는 조용히 통과하므로, 대개 이 스킬을 직접 부를 일은 **지금 당장 등록이 필요할 때** 뿐이다.
...
1. Obsidian 프로세스 실행 감지 — 실행 중이면 중단. **볼트 생성 플로우 중이라면 여기서 사용자를 붙잡지 말 것** — "다음에 Obsidian 닫으시면 세션 시작 시 자동 등록됩니다" 안내 후 진행 (`/create-vault` § 5-2). 지금 당장 필요할 때만 종료 요청하거나 `/obsidian-windows` 로 graceful close.
...
- **실행 중 Obsidian 차단**: 리서치 D1 확인 — live write-back 으로 변경 사일런트 revert 위험. 2026-08-26 에 두 번째 이유 확인 — 실행 중 인스턴스는 새 항목을 읽지 못하므로 강행 자체가 무의미.
